...
export class PasteActions {
    public static getPreviousImageLabel(): void {
        const currentImageIndex: number = LabelsSelector.getActiveImageIndex();
        const activeLabelId: string = LabelsSelector.getActiveLabelId();
        PasteActions.getImageByIndex(currentImageIndex-1 , activeLabelId);
      }

    
  public static getImageByIndex(index: number, previouslabel: string): void {
    if (EditorModel.viewPortActionsDisabled) return;

    const imageCount: number = LabelsSelector.getImagesData().length;
  //  const imgdata :string = this.getPreviousImageLabel
   // console.log(previouslabel,index,imgdata)
  ...
  public static setActiveLabelOnActiveImage(labelIndex: number): void {
    ...
    store.dispatch(
      updateImageDataById(
        imageData.id,
        PasteActions.mapNewImageData(imageData, labelIndex)
      )
    );
